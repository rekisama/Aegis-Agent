...
class DynamicToolCreator:
    ...
    def create_tool(self, name: str, description: str, code: str, 
        ...
        try:
            ...
            agent = Agent()
            fixed_code = agent._fix_unicode_characters(code)
            
            if fixed_code != code:
                logging.info(f"修复了工具 {name} 代码中的Unicode字符问题")
                code = fixed_code
        ...
        try:
            ...
            if success:
                ...
                try:
                    ...
                    # 将错误信息存储到工具规范中，供Agent使用
                    if hasattr(self, '_last_error_info'):
...
'''
        ...
        return fixed_template.format(name=name, name_capitalized=name.capitalize(), code=fixed_code)
